...
static const char *radio_access_mode_to_string(enum ofono_radio_access_mode m)
{
	if (m <= OFONO_RADIO_ACCESS_MODE_LTE)
	{
		...
		switch ((enum ofono_extended_radio_access_mode)m) {
...

{
	if (g_str_equal(str, "any")) {
		...
		*mode = OFONO_RADIO_ACCESS_MODE_LTE;
		return TRUE;
	} else if (g_str_equal(str, "gsm-umts")) {
		*mode = OFONO_RADIO_ACCESS_MODE_GSM_UMTS;
		return TRUE;
	} else if (g_str_equal(str, "gsm-lte")) {
		*mode = OFONO_RADIO_ACCESS_MODE_GSM_LTE;
		return TRUE;
	} else if (g_str_equal(str, "umts-lte")) {
