...
enum GSM_TELIT_STATE {
	GSM_TELIT_STATE_IDLE = 0,
	GSM_TELIT_STATE_RESPONSE,
	GSM_TELIT_STATE_RESPONSE_STRING,
	GSM_TELIT_STATE_GUESS_PDU,
	GSM_TELIT_STATE_PDU,
	GSM_TELIT_STATE_PROMPT,
	GSM_TELIT_STATE_GUESS_SHORT_PROMPT,
	GSM_TELIT_STATE_SHORT_PROMPT,
	GSM_TELIT_STATE_PPP_DATA,
};
...
static void gsm_telit_hint(GAtSyntax *syntax, GAtSyntaxExpectHint hint)
{
	if (hint == G_AT_SYNTAX_EXPECT_PDU)
		syntax->state = GSM_TELIT_STATE_GUESS_PDU;
	else if (hint == G_AT_SYNTAX_EXPECT_SHORT_PROMPT)
		syntax->state = GSM_TELIT_STATE_GUESS_SHORT_PROMPT;
}

static GAtSyntaxResult gsm_telit_feed(GAtSyntax *syntax,
										const char *bytes, gsize *len)
{
	gsize i = 0;
	GAtSyntaxResult res = G_AT_SYNTAX_RESULT_UNSURE;
	...
	while (i < *len) {
		...
		case GSM_TELIT_STATE_IDLE:
			if (byte == '\r' || byte == '\n')
				...
				syntax->state = GSM_TELIT_STATE_PPP_DATA;
			else if (byte == '"')
...
GAtSyntax *g_at_syntax_new_gsm_telit(void)
{
	return g_at_syntax_new_full(gsm_telit_feed, gsm_telit_hint,
				GSM_TELIT_STATE_IDLE);
}
