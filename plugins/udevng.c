...
static gboolean setup_ge910(struct modem_info *modem)
{
	...
	for (list = modem->devices; list; list = list->next) {
		...
		if (g_strcmp0(info->interface, "2/2/0") == 0) {
			if (g_strcmp0(info->number, "02") == 0)
				mdm = info->devnode;
			else if (g_strcmp0(info->number, "00") == 0)
				aux = info->devnode;
...
static gboolean setup_telit_qc(struct modem_info *modem)
{
	const char *mdm = NULL, *aux = NULL, *gps = NULL;
	GSList *list;
	...
	for (list = modem->devices; list; list = list->next) {
		...
		DBG("%s %s %s %s %s", info->devnode, info->interface,
						info->number, info->label, modem->model);

		if (g_strcmp0(info->interface, "255/0/0") == 0 &&
			g_strcmp0(modem->model, "1201") == 0)
		{
			if (g_strcmp0(info->number, "05") == 0)
				mdm = info->devnode;
			else if (g_strcmp0(info->number, "04") == 0)
...
static struct {
	...
	{ "he910",	setup_he910	},
	{ "le910v2",	setup_le910v2	},
	{ "ge910",	setup_ge910	},
	{ "telit_qc",	setup_telit_qc	},
	{ "sim900",	setup_sim900	},
...
static struct {
	...
	{ "le910v2",	"cdc_acm",	"1bc7", "0032"	},
	{ "le910v2",	"cdc_acm",	"1bc7", "0035"	},
	{ "le910v2",	"cdc_acm",	"1bc7", "0036"	},
	{ "ge910",	"cdc_acm",	"1bc7", "0022"	},
	{ "telit_qc",	"option",	"1bc7", "1201"	},
	{ "telit_qc",	"option",	"1bc7", "1012"	},
	{ "telit_qc",	"option",	"1bc7", "1010"	},
