...
static void telit_qc_debug(const char *str, void *user_data)
{
				...
				const char *key, char *debug)
{
	...
	if (getenv("OFONO_AT_DEBUG"))
		g_at_chat_set_debug(chat, telit_qc_debug, debug);
...
{
	struct telit_qc_data *data = ofono_modem_get_data(modem);
...
static void telit_qc_qss_notify(GAtResult *result, gpointer user_data)
{
...
static void cfun_enable_cb(gboolean ok, GAtResult *result, gpointer user_data)
{
	...
	/* Follow sim state */
	g_at_chat_register(data->chat, "#QSS:", telit_qc_qss_notify,
				FALSE, modem, NULL);
...
static int telit_qc_enable(struct ofono_modem *modem)
{
	struct telit_qc_data *data = ofono_modem_get_data(modem);
...
static int telit_qc_disable(struct ofono_modem *modem)
{
	struct telit_qc_data *data = ofono_modem_get_data(modem);
...
static void telit_qc_pre_sim(struct ofono_modem *modem)
{
	struct telit_qc_data *data = ofono_modem_get_data(modem);
...
static void telit_qc_post_online(struct ofono_modem *modem)
{
	struct telit_qc_data *data = ofono_modem_get_data(modem);
#ifndef DISABLE_VOICE_AND_SMS
...
static int telit_qc_probe(struct ofono_modem *modem)
{
	struct telit_qc_data *data;

	DBG("%p", modem);

	data = g_try_new0(struct telit_qc_data, 1);
	if (data == NULL)
		return -ENOMEM;
...
static void telit_qc_remove(struct ofono_modem *modem)
{
	struct telit_qc_data *data = ofono_modem_get_data(modem);
...
static struct ofono_modem_driver telit_qc_driver = {
	.name		= "telit_qc",
	.probe		= telit_qc_probe,
	.remove		= telit_qc_remove,
	.enable		= telit_qc_enable,
	.disable	= telit_qc_disable,
	.pre_sim	= telit_qc_pre_sim,
	.post_online	= telit_qc_post_online,
};

static int telit_qc_init(void)
{
	DBG("");

	return ofono_modem_driver_register(&telit_qc_driver);
}

static void telit_qc_exit(void)
{
	ofono_modem_driver_unregister(&telit_qc_driver);
}

OFONO_PLUGIN_DEFINE(telit_qc, "Telit TELIT_QC driver", VERSION,
		OFONO_PLUGIN_PRIORITY_DEFAULT, telit_qc_init, telit_qc_exit)
