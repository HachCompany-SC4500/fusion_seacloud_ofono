...
static void cfun0_cb(gboolean ok, GAtResult *result, gpointer user_data)
{
	struct ofono_modem *modem = user_data;
	struct huawei_data *data = ofono_modem_get_data(modem);
...
static void simst_notify(GAtResult *result, gpointer user_data)
{
	...
	/*
	* HUAWEI MS2131 sends a ^SIMST response with 255 indicating sim was
	* unplugged from modem. To ensure that the modem will not be powered off
	* by the connection manager in this state, we are performing CFUN = 0 and
	* CFUN = 1 to safely power cycle modem and updating have_sim state to
	* FALSE. 
	*/
	if(sim_state == 255) {
		data->have_sim = FALSE;
						...
						gpointer user_data)
{
	...
	/* Originally the AlwaysOn was only used for CDMA communication.
	 * To ensure consistency with the Telit driver, we are actually doing it for all
...
failure:
	/* To be able to react on no-sim-card scenario, we actually don't shutdown
	 * the device in these cases. Instead we manually power the modem and skip
	 * the actual shutdown. */
	// shutdown_device(data);
	ofono_info("Setup problem / No Sim card present");
	ofono_modem_set_powered(modem, TRUE);
}
...
static void rfswitch_support(gboolean ok, GAtResult *result, gpointer user_data)
{
	...
	if (strcmp(model, "MS2131") == 0) {
		/* Original ofono implementation sets CFUN=5 for all huawei modems.
		 * We try to avoid this for the MS2131, since the AT Command
		 * Guide lists value 5 as 'reserved' for this model. */
		data->offline_command = "AT+CFUN=4";
		goto done;
	} else if (data->have_gsm == FALSE && data->have_cdma == TRUE) {
...
static void gcap_support(gboolean ok, GAtResult *result, gpointer user_data)
{
	...
	if (!g_at_result_iter_next(&iter, NULL))
		ofono_warn("Could not read first ATI response line");

	if (!g_at_result_iter_next(&iter, "Model:"))
		ofono_warn("Could not parse model from ATI response");

	g_at_result_iter_next_unquoted_string(&iter, &model);
	ofono_modem_set_string(modem, "Model", model);
...
static int huawei_enable(struct ofono_modem *modem)
{
	...
	g_at_chat_send(data->pcui, "ATI", NULL,
